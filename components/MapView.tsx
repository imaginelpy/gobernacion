...
export function MapView({ obras, canEdit }: { obras: any[]; canEdit: boolean }) {
    ...
    <div className="card">
      ...
      <div className="h-[420px] rounded-lg overflow-hidden">
        <MapContainer center={[first.latitud, first.longitud]} zoom={9} className="h-full w-full">
          ...
          <FlyToMarker lat={first.latitud} lng={first.longitud} />
          {obras.map((obra) => (
            <Marker key={obra.id} position={[obra.latitud, obra.longitud]} icon={icon}>
              <Popup>
